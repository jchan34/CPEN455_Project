...
if __name__ == '__main__':
    ...
    if "mnist" in args.dataset:
        ...
        computed_train_mean = [0.5045, 0.4274, 0.3339]
        computed_train_std = [0.2474, 0.2287, 0.2321]
        ds_transforms = transforms.Compose([transforms.Resize((32, 32)),
                                            ...
                                            rescaling,
                                            transforms.Normalize(mean=computed_train_mean, std=computed_train_std),
                                        ])

        
        computed_val_mean = [0.5043, 0.4277, 0.3351]
        computed_val_std = [0.2481, 0.2294, 0.2325]
       
        val_ds_transforms = transforms.Compose([transforms.Resize((32, 32)),
                                            rescaling,
                                            transforms.Normalize(mean=computed_val_mean, std=computed_val_std),
                                        ])
    ...
    model = PixelCNNCond(nr_resnet=args.nr_resnet, nr_filters=args.nr_filters, 
                input_channels=input_channels, nr_logistic_mix=args.nr_logistic_mix, device=device, num_classes=4)
