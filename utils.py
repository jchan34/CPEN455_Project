...
def concat_elu(x):
    ...
    # Pytorch ordering
    axis = len(x.size()) - 3
    return F.elu(torch.cat([x, -x], dim=axis).to(x.device))
...
def log_sum_exp(x):
    ...
    return m + torch.log(torch.sum(torch.exp(x - m2), dim=axis)).to(x.device)
...
def discretized_mix_logistic_loss(x, l, sum_over_batch=True):
    ...
    if sum_over_batch:
        ...
        return torch.sum(log_sum_exp(log_probs),dim=(1,2)).unsqueeze(1)
...
def sample_from_discretized_mix_logistic(l, nr_mix):
    ...
    out = torch.cat([x0.view(xs[:-1] + [1]), x1.view(xs[:-1] + [1]), x2.view(xs[:-1] + [1])], dim=3).to(l.device)
...
def sample(model, sample_batch_size, obs, sample_op):
    ...
    with torch.no_grad():
        ...
        labels = torch.randint(0, 4, (sample_batch_size,)).to(device)
        data = data.to(device)
...
def save_images(tensor, images_folder_path,labels=''):
    os.makedirs(images_folder_path, exist_ok=True)
    if os.path.exists(images_folder_path):
        num_images = len(os.listdir(images_folder_path))
        num = num_images // 4
    for i, img_label_tuple in enumerate(zip(tensor,labels)):
        img_tensor,label = img_label_tuple
        img = Image.fromarray((img_tensor.cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8), mode='RGB')
        img_path = f"{images_folder_path}/{label}_image_{i+1 + num:02d}.jpg"
        img.save(img_path)